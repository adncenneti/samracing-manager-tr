...
export default function App() {
  ...
  const mapBlacklist = (b: any): BlacklistEntry => ({
    id: b.id,
    phone: b.phone,
    reason: b.reason,
    name: b.name,
      ...
      <div className="w-full h-full flex bg-sim-dark border-4 border-sim-yellow rounded-xl shadow-2xl overflow-hidden relative" onClick={() => setSelectedIds([])}>
        ...
        <div className="flex-1 flex flex-col min-w-0 border-r border-sim-border">
          <header className="flex items-center justify-between px-6 py-4 border-b border-sim-border bg-[#121212] shrink-0 z-[70]">
            <div className="flex items-center gap-8">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <div onClick={(e) => {e.stopPropagation(); dateInputRef.current?.showPicker()}} className="flex flex-col items-center px-4 cursor-pointer group relative">
                        ...
                        <span className="text-sm font-medium text-gray-200">{format(currentDate, 'dd MMMM yyyy', { locale: tr })}</span>
            ...
            <div className="flex items-center gap-3 relative">
                ...
                <button onClick={(e) => {e.stopPropagation(); setIsSettingsModalOpen(true)}} className="p-2 text-sim-yellow hover:bg-sim-yellow/10 rounded-lg border border-sim-yellow/20"><Settings size={20} /></button>
                <button onClick={(e) => {e.stopPropagation(); setIsBlacklistOpen(true)}} className="p-2 text-sim-yellow hover:bg-sim-yellow/10 rounded-lg border border-sim-yellow/20"><AlertOctagon size={20} /></button>
                
                <div className="flex bg-sim-black p-1 rounded-lg border border-sim-border">
                    {simulatorGroups.map(group => (
                        <button key={group.id} onClick={(e) => {e.stopPropagation(); setView(group.id)}} className={`px-4 py-1.5 rounded-md font-bold text-[10px] uppercase transition-all ${view === group.id ? 'bg-sim-yellow text-black' : 'text-gray-500'}`}>{group.name}</button>
                    ))}
                </div>

                <button onClick={(e) => {e.stopPropagation(); setIsModalOpen(true)}} className="bg-sim-yellow text-black px-5 py-2 rounded-lg font-black text-[11px] uppercase flex items-center gap-2"><Plus size={14}/> Yeni</button>
            </div>
          </header>

          <div className="flex-1 overflow-auto relative bg-sim-black">
             <div className="flex relative" style={{ height: totalContentHeight }}>
                 <div className="w-14 shrink-0 bg-sim-dark border-r border-sim-border sticky left-0 z-[65]">
                    <div className="h-[40px] border-b border-sim-yellow flex items-center justify-center font-bold text-[8px] text-sim-yellow uppercase sticky top-0 bg-sim-dark">TIME</div>
                 ...
                 <div className="flex-1 flex min-w-[800px] relative">
                    {isSameDay(currentDate, now) && (
                      <div className="absolute left-0 right-0 bg-sim-yellow/15 border-b-2 border-sim-yellow/60 pointer-events-none z-[50]" style={{ top: 40, height: `${Math.max(0, (((now.getHours() * 60 + now.getMinutes()) - START_HOUR * 60) / (totalHours * 60)) * 100)}%` }}>
                         <div className="absolute bottom-0 right-2 bg-sim-yellow text-black text-[8px] font-black px-1.5 py-0.5 rounded">ŞUAN</div>
                      </div>
                    )}
                    ...
                    {currentViewSeats.map((seat) => (
                        <div key={seat.id} className="flex-1 border-r border-sim-border/20 relative flex flex-col">
                            ...
                            <div className="relative flex-1 w-full" onDragOver={(e) => handleDragOver(e, seat.id)} onDrop={(e) => handleDrop(e, seat.id)}>
                                <div className="absolute inset-0 z-10" onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); const y = e.clientY - e.currentTarget.getBoundingClientRect().top; const t = minutesToTime(Math.floor(((START_HOUR * 60) + (y / HOUR_HEIGHT * 60)) / 30) * 30); setContextMenu({ visible: true, x: e.pageX, y: e.pageY, seatId: seat.id, time: t }); }} />
                                ...
                                {reservations.filter(r => r.seatId === seat.id && r.date === dateStr).map(res => {
                                    ...
                                    const status = now.getHours() * 60 + now.getMinutes() > timeToMinutes(res.endTime) ? 'PAST' : 'FUTURE';
                                    ...
                                    return (
                                        <div 
                                          ...
                                          className={`absolute left-0 right-0 rounded-sm overflow-hidden flex flex-col justify-center text-center p-1 transition-all border z-[50] ${selected ? 'border-white ring-4 ring-sim-yellow/50 scale-[1.05] z-[70]' : ''} ${res.isPaid ? 'bg-emerald-600' : 'bg-red-600 animate-pulse'} ${status === 'PAST' ? 'opacity-40 grayscale' : ''}`}
                                          style={{ top: pos.top, height: pos.height }}
                                        >
                                            <div className="text-[10px] font-bold opacity-90">{res.startTime}-{res.endTime}</div>
                                            <div className="font-black text-[12px] truncate uppercase">{res.name}</div>
                                        </div>
                                    );
                                })}
                            </div>
        ...
        <aside className="w-[320px] bg-sim-dark/50 backdrop-blur-xl flex flex-col shrink-0 border-l border-sim-border z-[70]" onClick={(e) => e.stopPropagation()}>
          <div className="p-5 border-b border-sim-border bg-sim-black/20 flex items-center justify-between">
            <div className="flex items-center gap-3 text-sim-yellow font-black text-xs uppercase"><Edit3 size={16}/> Düzenle</div>
            {selectedIds.length > 0 && <span className="bg-sim-yellow text-black text-[9px] font-black px-2 py-0.5 rounded-full">{selectedIds.length} SEÇİLİ</span>}
          </div>
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {selectedIds.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center opacity-10 text-center"><MousePointer2 size={64} /><p className="text-[10px] font-bold uppercase mt-4">Bir kayıt seçin</p></div>
            ) : (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="space-y-4">
                    <input type="text" value={editForm.name} onChange={(e) => setEditForm({...editForm, name: e.target.value.toUpperCase()})} className="w-full bg-sim-black border border-sim-border rounded-lg p-3 text-xs text-white uppercase outline-none focus:border-sim-yellow" placeholder="MÜŞTERİ" />
                    <input type="text" value={editForm.phone} onChange={(e) => setEditForm({...editForm, phone: formatPhoneNumber(e.target.value)})} className="w-full bg-sim-black border border-sim-border rounded-lg p-3 text-xs text-white outline-none focus:border-sim-yellow" placeholder="TELEFON" />
                    <div className="grid grid-cols-2 gap-4">
                        <input type="time" value={editForm.startTime} onChange={(e) => setEditForm({...editForm, startTime: e.target.value})} className="bg-sim-black border border-sim-border rounded-lg p-3 text-xs text-white" />
                        <input type="time" value={editForm.endTime} onChange={(e) => setEditForm({...editForm, endTime: e.target.value})} className="bg-sim-black border border-sim-border rounded-lg p-3 text-xs text-white" />
                    </div>
                    <div className="space-y-2">
                        <div className="flex items-center justify-between"><label className={`text-[9px] font-black uppercase tracking-widest ${isGroupMode ? 'text-green-500' : 'text-gray-500'}`}>{isGroupMode ? "GRUP MODU: EKLE/ÇIKAR" : "MASA DEĞİŞTİR"}</label><button onClick={() => setIsGroupMode(!isGroupMode)} className={`text-[8px] font-bold px-2 py-1 rounded border ${isGroupMode ? 'bg-green-600 border-green-500' : 'bg-sim-dark border-sim-border'}`}>{isGroupMode ? "MOD: GRUP" : "MOD: TEK"}</button></div>
                        <div className={`grid grid-cols-4 gap-2 p-3 rounded-xl border-2 transition-all ${isGroupMode ? 'border-green-500' : 'border-sim-border'}`}>
                            {seats.filter(s => s.type === view).map(s => {
                                const isSel = selectedIds.some(id => reservations.find(r => r.id === id)?.seatId === s.id);
                                return <button key={s.id} onClick={() => isGroupMode ? handleGroupSeatToggle(s.id) : setEditForm({...editForm, seatId: s.id})} className={`p-2 text-[8px] font-black rounded border ${isSel ? 'bg-sim-yellow text-black' : 'bg-sim-gray text-gray-400 border-sim-border'}`}>{s.label}</button>
                            })}
                        </div>
                    </div>
                    <div className="flex bg-sim-black p-1 rounded-xl border border-sim-border">
                        <button onClick={() => setEditForm({...editForm, isPaid: true})} className={`flex-1 py-3 text-[10px] font-black rounded-lg ${editForm.isPaid ? 'bg-green-600 text-white' : 'text-gray-600'}`}>ALINDI</button>
                        <button onClick={() => setEditForm({...editForm, isPaid: false})} className={`flex-1 py-3 text-[10px] font-black rounded-lg ${!editForm.isPaid ? 'bg-red-900 text-white' : 'text-gray-600'}`}>ALINMADI</button>
                    </div>
                  </div>
                  <div className="flex flex-col gap-3">
                    <button onClick={handleSaveEdit} className="w-full bg-sim-yellow text-black font-black py-4 rounded-xl text-[11px] uppercase flex items-center justify-center gap-2 shadow-lg shadow-yellow-500/20"><Save size={16} /> Güncelle</button>
                    <button onClick={async () => { if(confirm('Silmek istediğine emin misin?')) { await Promise.all(selectedIds.map(id => supabase.from('reservations').delete().eq('id', id))); setSelectedIds([]); } }} className="w-full bg-transparent border border-red-900/50 text-red-700 font-black py-3 rounded-xl text-[10px] uppercase">Sil</button>
                  </div>
              </div>
            )}
          </div>
...
function LoginScreen() {
    ...
    return (
        <div className="fixed inset-0 bg-sim-black flex items-center justify-center p-4">
            <div className="w-full max-w-[400px] bg-sim-dark border border-sim-yellow/30 p-10 rounded-2xl shadow-2xl relative overflow-hidden">
                ...
                <div className="flex flex-col items-center mb-8">
                    ...
                    <h2 className="text-sim-yellow font-black text-xs tracking-[0.3em] uppercase">SimRacing Manager</h2>
                </div>
                <form onSubmit={handleLogin} className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full bg-sim-black border border-sim-border rounded-xl p-4 text-sm text-white outline-none focus:border-sim-yellow transition-all" placeholder="admin@simracing.com" />
                    </div>
                    <div className="space-y-2">
                        ...
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full bg-sim-black border border-sim-border rounded-xl p-4 text-sm text-white outline-none focus:border-sim-yellow transition-all" placeholder="••••••••" />
                    </div>
                    {error && <div className="text-red-500 text-[10px] font-bold text-center uppercase tracking-wider animate-pulse">{error}</div>}
                    <button type="submit" disabled={loading} className="w-full bg-sim-yellow text-black font-black py-4 rounded-xl text-xs uppercase hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2">
